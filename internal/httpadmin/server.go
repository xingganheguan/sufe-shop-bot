...
type Server struct {
	...
	adminTOTPSecretFile string
}

func NewServer(adminToken string, db *gorm.DB) *Server {
	adminTOTPEnabled := isEnvTrue(os.Getenv("ADMIN_TOTP_ENABLED"))
	adminTOTPSecretFile := strings.TrimSpace(os.Getenv("ADMIN_TOTP_SECRET_FILE"))
	var adminTOTPSecret string

	if adminTOTPEnabled {
		var secretErr error
		adminTOTPSecret, secretErr = loadSecretFromFile(adminTOTPSecretFile)
		if secretErr != nil {
			logger.Error("Failed to load admin TOTP secret", "error", secretErr, "secret_file", adminTOTPSecretFile)
	...
	if err != nil {
		...
		return &Server{
			...
			db:                  db,
			adminTOTPEnabled:    adminTOTPEnabled,
			adminTOTPSecret:     adminTOTPSecret,
			adminTOTPSecretFile: adminTOTPSecretFile,
		}
	}
...
func (s *Server) handleLoginPage(c *gin.Context) {
	c.HTML(http.StatusOK, "login.html", gin.H{
		"totp_enabled": s.adminTOTPEnabled,
	})
}
...
func (s *Server) handleLogin(c *gin.Context) {
	...
	if strings.TrimSpace(req.Token) != strings.TrimSpace(s.adminToken) {
	...
	if s.adminTOTPEnabled {
		...
		if otpCode == "" {
			...
			if s.securityLogger != nil {
				s.securityLogger.LogLoginFailed("admin", clientIP, userAgent, "missing_otp")
			}
			JSONError(c, NewUnauthorizedError("2FA code required"))
			return
		}

		if !totp.Validate(otpCode, s.adminTOTPSecret) {
			if s.rateLimiter != nil {
				s.rateLimiter.RecordAttempt(clientIP, false)
			}
			if s.securityLogger != nil {
				s.securityLogger.LogLoginFailed("admin", clientIP, userAgent, "invalid_otp")
			}
			JSONError(c, NewUnauthorizedError("Invalid 2FA code"))
			return
		}
	}

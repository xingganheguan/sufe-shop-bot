...
func ExpirePendingOrders(db *gorm.DB) error {
    // Get expiration minutes setting
    expireMinutesStr, err := GetSetting(db, SettingOrderExpireMinutes)
    if err != nil {
        return err
    }

    expireMinutes, err := strconv.Atoi(expireMinutesStr)
    if err != nil || expireMinutes < 10 {
        expireMinutes = 10 // Default to 10 minutes
    }
